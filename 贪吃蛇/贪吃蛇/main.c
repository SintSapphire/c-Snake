...
int main(void)
{
	...
	g_pHead;
	CreateSnake(3);
	//GotoXY(unsigned short hang, unsigned short lie);
	DrawSnake();
	Food();
	SetScore();
	while (1)
...
void Food(void)
{
	while (1)
	{
		...
		int x = rand() % (X-2) + 1;
		int y = rand() % (Y-2) + 1;
		//判断是否在蛇身上  在的话就下一次循环  不在，那就是合理的位置
...
void CreateSnake(int iCount)
{
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (NULL == g_pHead)
		{
			...
			g_pEnd = pT;
			g_pHead->iCount += 1;
			g_pHead->stSnake.iX = rand()%(X-3)+1;
			g_pHead->stSnake.iY = rand() % (Y - iCount - 2) + 1;
		}
		else
		{
			g_pEnd->pNext = pT;
			...
			//g_pEnd->stSnake.iX = rand() % (X - 3) + 1;
			g_pEnd->stSnake.iX = g_pEnd->pPre->stSnake.iX;

			//g_pEnd->stSnake.iY = rand() % (Y - iCount - 2) + 1;
